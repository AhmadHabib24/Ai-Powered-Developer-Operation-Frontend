...
export default function LoginPage() {
  ...
  return (
    <div className="mx-auto grid min-h-screen max-w-md place-items-center p-6">
      <Card className="w-full space-y-6 border-amber-400/25 shadow-[0_0_60px_rgba(245,158,11,0.12)]">
        <div className="text-center">
          ...
          <p className="text-[10px] uppercase tracking-[0.32em] text-amber-700 dark:text-amber-300">{BRAND.tagline}</p>
          <h1 className="mt-2 text-2xl font-semibold text-foreground">Sign in to {appName}</h1>
          <p className="mt-1 text-sm text-muted">Command center and developer workspace.</p>
        </div>
        <form className="space-y-4" onSubmit={form.handleSubmit((values) => mutation.mutate(values))}>
          <div>
            <label className="mb-1 block text-sm text-muted">Email</label>
            <Input type="email" placeholder="cto@nova.test" {...form.register("email")} />
            {form.formState.errors.email && <p className="mt-1 text-xs text-rose-700 dark:text-rose-300">{form.formState.errors.email.message}</p>}
          </div>
          <div>
            <label className="mb-1 block text-sm text-muted">Password</label>
            <Input type="password" {...form.register("password")} />
            {form.formState.errors.password && <p className="mt-1 text-xs text-rose-700 dark:text-rose-300">{form.formState.errors.password.message}</p>}
          </div>
